...
[ncf](http://www.ncf.io) is a framework that runs in __pure CFEngine language__, to help __structure__ your CFEngine policy and provide __reusable, single purpose components__ distributed under the __GPLv3__ license.
...
To test ncf immediately, just do the following, after installing CFEngine:

    cd $(mktemp -d)
    git clone https://github.com/Normation/ncf.git
    cp -a ncf/tree/* /var/cfengine/inputs/

and run cf-agent:

    /var/cfengine/bin/cf-agent -KI

You can then take a look at 60_services/baseline/stable/baseline.cf inside /var/cfengine/inputs to begin with (this is where ncf loads the base promises to apply on a machine by default).

The default "ntp" bundle / Technique is in 50_techniques/ntp/ntp.cf, and uses "generic methods" available in 30_generic_methods, as seen in the previous example.
...
The main web site is http://www.ncf.io/

We'd love to hear about your feedback/issues/bugs on http://www.rudder-project.org/redmine/projects/ncf

You can also get in touch with us on IRC ([#ncf on Freenode](irc://#irc@irc.freenode.net))!
